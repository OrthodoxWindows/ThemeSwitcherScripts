...
echo * Type de thème : DWM-Effect, style visuel Windows 10 Aero (Autocolorization) *
echo * Paramètres définis : Thème par défaut dans Windows 10,                      *
echo * avec un diaporama d'images de fleurs en arrière-plan                        *
echo *******************************************************************************
echo.

:APPLY
set /P c=Voulez-vous appliquer le thème Windows 10 - Fleurs ?[Y\N]?
if /I "%c%" EQU "Y" (call "%userprofile%\TSS\Switcher_Scripts\DWM-Effects.bat"
goto :THM)
if /I "%c%" EQU "N" (goto :ABORT)

:THM
"%userprofile%\TSS\WTS\ThemeSwitcher.exe" "C:\Windows\Resources\Themes\theme2.theme"
