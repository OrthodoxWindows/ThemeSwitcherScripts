...
for /f "tokens=3 " %%i in ('reg query "HKEY_LOCAL_MACHINE\SOFTWARE\Windhawk\Engine\Mods\basic-classic-theme-ribbon-fix" /v "disabled" ^| find "0x0" ') do REG ADD HKEY_LOCAL_MACHINE\SOFTWARE\Windhawk\Engine\Mods\basic-classic-theme-ribbon-fix /v Disabled /t REG_DWORD /d 1 /f
