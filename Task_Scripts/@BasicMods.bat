...
for /f "tokens=3 " %%i in ('reg query "HKEY_LOCAL_MACHINE\SOFTWARE\Windhawk\Engine\Mods\dwm-ghost-mods\Settings" /v "useClassicTheme" ^| find "0x1" ') do REG ADD HKEY_LOCAL_MACHINE\SOFTWARE\Windhawk\Engine\Mods\dwm-ghost-mods\Settings /v useClassicTheme /t REG_DWORD /d 0 /f
...
for /f "tokens=3 " %%i in ('reg query "HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer" /v "AltTabSettings" ^| find "0x1" ') do REG ADD HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer /v AltTabSettings /t REG_DWORD /d 0 /f
...
::for /f "tokens=3 " %%i in ('reg query "HKEY_LOCAL_MACHINE\SOFTWARE\Windhawk\Engine\Mods\aerexplorer\Settings" /v "alwayscpl" ^| find "0x1" ') do REG ADD HKEY_LOCAL_MACHINE\SOFTWARE\Windhawk\Engine\Mods\aerexplorer\Settings /v alwayscpl /t REG_DWORD /d 0 /f
...
::Mods Msstyle
for /f "tokens=3 " %%i in ('reg query "HKEY_LOCAL_MACHINE\SOFTWARE\Windhawk\Engine\Mods\themed-regedit-listview" /v "disabled" ^| find "0x1" ') do REG ADD HKEY_LOCAL_MACHINE\SOFTWARE\Windhawk\Engine\Mods\themed-regedit-listview /v Disabled /t REG_DWORD /d 0 /f
...
for /f "tokens=3 " %%i in ('reg query "HKEY_CURRENT_USER\SOFTWARE\OpenShell\StartMenu\Settings" /v "CustomTaskbar" ^| find "0x1" ') do REG ADD HKEY_CURRENT_USER\SOFTWARE\OpenShell\StartMenu\Settings /v CustomTaskbar /t REG_DWORD /d 0 /f
