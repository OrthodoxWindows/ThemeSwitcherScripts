...
for /f "tokens=3 " %%i in ('reg query "HKEY_CURRENT_USER\SOFTWARE\ThemeSwitcherScripts\Profiles" /v "Basic-Classic" ^| find "0x0" ') do (REG ADD HKEY_CURRENT_USER\SOFTWARE\ThemeSwitcherScripts\Profiles /v Basic-Classic /t REG_DWORD /d 1 /f
call "%userprofile%\tss\task_scripts\@BasicClassicMods.bat")
